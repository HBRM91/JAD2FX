...
import { ViewState, BasketConfig } from './types';
import { calculateFairValue } from './utils/fairValue';
import { MARKET_NEWS, DISCLAIMER_TEXT } from './constants';
...
function App() {
  ...
  useEffect(() => {
    ...
    const config: BasketConfig = {
      eurWeight: 0.6,
      usdWeight: 0.4,
      referenceBasketValue: 10.95,
      spreadPercent: 0.05,
      lastUpdated: new Date().toISOString()
    };
    const results = calculateFairValue(1.0850, config);
    setFairValue(results);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-800">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                        <div className="bg-navy-900 p-8 text-white relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-64 h-64 bg-gold-500 opacity-5 rounded-full blur-3xl transform translate-x-1/2 -translate-y-1/2"></div>
                            ...
                            <p className="text-slate-300 mb-6 max-w-lg relative z-10">
                                As global liquidity shifts, Bank Al-Maghrib's recent stance suggests a stable band for Q4. 
                                Find out how this impacts your import obligations.
                            </p>
                            <button onClick={() => setView('ANALYSIS')} className="flex items-center gap-2 text-gold-400 font-bold text-sm hover:text-white transition">
                            ...
                            {MARKET_NEWS.map((news) => (
                                <div key={news.id} className="group cursor-pointer border-b border-slate-100 pb-6 last:border-0 last:pb-0">
                                    <div className="flex items-center gap-3 mb-2">
                                        <span className="text-[10px] font-bold uppercase tracking-wider text-gold-600 bg-gold-50 px-2 py-0.5 rounded-full">{news.category}</span>
                                        <span className="text-xs text-slate-400">{news.time}</span>
                                    </div>
                                    <h4 className="text-lg font-semibold text-navy-900 group-hover:text-gold-600 transition mb-1">{news.title}</h4>
                ...
                <div className="lg:col-span-1">
                    <div className="sticky top-24">
                        <ChatInterface />
                        <div className="mt-4 p-4 bg-white rounded-lg border border-slate-200 shadow-sm text-center">
        ...
        {view === 'DASHBOARD' && <FxDashboard fairValue={fairValue} />}
      ...
      <footer className="bg-navy-900 text-slate-400 py-8 border-t border-navy-800">
        <div className="max-w-7xl mx-auto px-4 text-center">
            ...
            <p className="text-[10px] leading-relaxed max-w-2xl mx-auto opacity-70">
                {DISCLAIMER_TEXT}
            </p>
