...
export default function VolSurfacePage() {
  const availableCurrencies = useMemo(() => {
    return getAllCurrenciesWithVol().filter((c) =>
      BKAM_CURRENCIES.some((b) => b.code === c),
    );
  }, []);
  const [currency, setCurrency] = useState(availableCurrencies[0] || 'EUR');
  const surface = useMemo(() => getVolSurface(currency), [currency]);
          ...
          <table className="w-full text-[10px]">
            ...
            <tbody className="divide-y divide-navy-800">
              {surface.map((d) => {
                ...
                const maxVol = Math.max(...d.smile.map((s) => s.vol));
                return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {surface.slice(0, 3).map((d) => (
            <div key={d.tenorDays} className="bg-navy-950 border border-navy-800 rounded-lg p-3">
              ...
              <div className="h-32">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={d.smile.map((s) => ({ strike: s.strike * 100, vol: s.vol * 100 }))}>
