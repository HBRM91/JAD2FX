...
const SPREAD_TOOLTIP = "Marges simulÃ©es sur base des moyennes mondiales marchÃ©s Ã©mergents. Sources : Banque Mondiale (Remittance Prices 2024) / BIS (Triennial Survey 2022). Non contractuelles.";
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        {isJumuahReducedLiquidity(rate.currency) && (
                          <span title="Vendredi â€” LiquiditÃ© rÃ©duite" className="text-[11px]">ðŸ•Œ</span>
                        )}
                        {meta?.bkamUnit === 100 && (
                          <span className="text-[8px] bg-navy-800 text-navy-400 border border-navy-700 px-1 rounded">Ã—100</span>
                        )}
                        {meta?.umaConvention && (
                          <span title="Taux fixÃ© par Convention UMA (Doc 1 Â§I.1.b) â€” non cotÃ© en cross ECB" className="text-[7px] bg-amber-950/30 text-amber-600 border border-amber-800/40 px-1 rounded">UMA</span>
                        )}
                      </div>
    ...
    return (
      <div className="p-5">
        <h4 className="text-[11px] font-bold text-white uppercase tracking-wider mb-3 flex items-center gap-2">
          ...
          {locale === 'en' ? 'Bank Comparison' : 'Comparatif Ã‰tablissements'} â€” {selectedRate.pair}
          ...
          <thead className="border-b border-navy-800">
            <tr>
              <th className="pb-2 text-navy-500 font-bold uppercase tracking-wider text-[9px]">
                {locale === 'en' ? 'Establishment' : 'Ã‰tablissement'}
              </th>
              <th className="pb-2 text-right text-navy-500 font-bold uppercase tracking-wider text-[9px]">{L('Achat RÃ©f.', 'Ref Bid', 'Ø³Ø¹Ø± Ø§Ù„Ø´Ø±Ø§Ø¡')}</th>
              <th className="pb-2 text-right text-navy-500 font-bold uppercase tracking-wider text-[9px]">{L('Vente RÃ©f.', 'Ref Ask', 'Ø³Ø¹Ø± Ø§Ù„Ø¨ÙŠØ¹')}</th>
              <th className="pb-2 text-right text-navy-500 font-bold uppercase tracking-wider text-[9px]">{L('vs RÃ©f.', 'vs Ref', 'Ù…Ù‚Ø§Ø¨Ù„ Ø§Ù„Ù…Ø±Ø¬Ø¹')}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-navy-800/60">
            <tr>
        ...
        <p className="text-[9px] text-navy-700 mt-3">
          Marges simulÃ©es â€” non contractuelles. Sources : BM / BIS Triennial Survey. Pour taux exÃ©cutables, contactez votre banque.
        </p>
      </div>
    );
  };
      ...
      <div className={`flex items-start gap-2 bg-amber-500/5 border border-amber-500/15 rounded-lg px-4 py-2.5 text-[11px] text-amber-400/80 ${isRTL ? 'flex-row-reverse text-right' : ''}`}>
        ...
        <span className="leading-relaxed">
          {locale === 'ar'
            ...
            : 'Taux indicatifs dÃ©rivÃ©s du panier BKAM (60% EUR / 40% USD). Non officiels Bank Al-Maghrib. '}
            ...
            <div className="px-5 py-3 border-b border-navy-800 flex items-center justify-between">
              <div>
                ...
                <p className="text-[9px] text-navy-500 mt-0.5">
                  {activeTab === 'VIREMENTS' ? 'Spread Â±0.8% indicatif' : 'Spread Â±1.8% indicatif'}
                  {' Â· '}{filteredRates.length} devises
                </p>
              </div>
          ...
          <div className="xl:col-span-1 space-y-4">
            ...
            <div className="bg-navy-900 border border-navy-800 rounded-xl p-4">
              ...
              <p className="text-[9px] text-navy-600 mb-3">
                {locale === 'en' ? 'USD cross-rates Â· indicative' : 'Taux croisÃ©s via USD Â· indicatif'}
              </p>
                    ...
                    >
                      <span className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          {meta?.bkamUnit === 100 && <span className="text-[8px] text-navy-600 ml-1">Ã—100</span>}
                        </div>
                      </span>
